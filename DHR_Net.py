...
class ResECA(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.conv1 = DepthWiseConv2d(in_c, out_c, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(out_c)

        self.conv2 = DepthWiseConv2d(out_c, out_c, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(out_c)

        self.conv3 = DepthWiseConv2d(in_c, out_c, kernel_size=1, padding=0)
...
class HABlock(nn.Module):
    def __init__(self, n_channels):
        ...
        self.CA = ChannelAttention(self.channel)
        self.SA = SpatialAttention(self.channel)
        self.relu = nn.ReLU()
    def forward(self, x):
        residual = x
...
# 8,16,24,32,48,64
# GFLOPs :0.08, Params : 0.20
# 8,16,32,48,64,96
# GFLOPs :0.18, Params : 0.54
# 16,24,32,48,64,128
# GFLOPs :0.30, Params : 0.74
...
class DHR_Net(nn.Module):
    def __init__(self, n_classes=1, n_channels=3, c_list=[8,16,24,32,48,64], bridge=True, gt_ds=True):
        ...
        self.encoder5 = nn.Sequential(
            DepthWiseConv2d(c_list[3], c_list[4], 3, stride=1, padding=1),
            HABlock(c_list[4]),
        )


        self.encoder6 = nn.Sequential(
            DepthWiseConv2d(c_list[4], c_list[5], 3, stride=1, padding=1),
            HABlock(c_list[5]),
        )
        ...
        self.decoder1 = nn.Sequential(
            HABlock(c_list[5]),
            DepthWiseConv2d(c_list[5], c_list[4], 3, stride=1, padding=1),
        )

        self.decoder2 = nn.Sequential(
            HABlock(c_list[4]),
            DepthWiseConv2d(c_list[4], c_list[3],3, stride=1, padding=1),
        )
